<?php
	...
	if(empty($user->rights->upbuttons->UseAllButton) && empty($user->rights->upbuttons->UseSingleButton)) exit;
...
?>$(document).ready(function() {
  ...
  if (editline_subtotal == -1 && ($upbuttons_container.length == 1 && ($upbuttons_container.find('.button').length>0 || $upbuttons_container.find('.butAction').length>0)))
  {
		...
		if(originalElementTop <= 0) {
			window.setTimeout(function() { originalElementTop = $upbuttons_container.offset().top;originalElementLeft = $upbuttons_container.offset().left; scrollButtonsToUp(); },100);
		}
